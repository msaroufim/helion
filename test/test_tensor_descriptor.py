...
class TestTensorDescriptor(RefEagerTestBase, TestCase):
    ...
    def test_multistage_tensor_descriptor_alignment_small_tile(self):
        """
        [4, 256] -> pass
        [8, 128] -> pass
        [16, 64] -> pass
        ...
        [4, 512] -> pass
        [8, 256] -> fail (but passes on H100)
        [16, 128] -> fail (but passes on H100)
        [32, 64] -> fail (but passes on H100)
        [64, 32] -> fail (but passes on H100)
        [128, 16] -> pass
        [256, 8] -> pass
        ...
        def jsd_forward_kernel(
            ...
            # Ensure the multistage ring stride stays below 4KB so we exercise the
            # tensor-descriptor path even with the current guard in
            # TensorDescriptorIndexingStrategy.
            # assert block_size_n * block_size_m * _input.element_size() < 4096
    ...
    def test_multistage_tensor_descriptor_alignment(self):
        @helion.kernel(
            config=helion.Config(
                block_sizes=[4, 256],
                indexing="tensor_descriptor",
    ...
    def test_multistage_tensor_descriptor_alignment_large_tile(self):
        @helion.kernel(
            config=helion.Config(
                block_sizes=[8, 256],
                indexing="tensor_descriptor",
        ...
        code_desc, result_desc = code_and_output(
            jsd_forward_kernel,
            (log_q, log_p),
        )
